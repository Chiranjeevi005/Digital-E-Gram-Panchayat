...
export default function OfficerApplicationDetail({ params }: PageProps) {
  ...
  const updateApplicationStatus = async (newStatus: 'pending' | 'in-progress' | 'approved' | 'rejected') => {
    if (!resolvedParams) return
    ...
    try {
      ...
      if (response.ok) {
        setApplication(prev => prev ? { ...prev, status: newStatus, remarks, assignedTo: data.assignedTo } : null)
      } else {
                  ...
                  {application?.assignedTo && (
                    <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                      <p className="text-sm text-blue-800">
                        Assigned to: <span className="font-medium">{application.assignedTo.name}</span>
                      </p>
                    </div>
                  )}
                </div>
            ...
            <div>
              <Card title="Update Application">
                <Form onSubmit={(e) => e.preventDefault()}>
                  ...
                  <Form.Field label="Assign To Staff" className="mt-4">
                    <Form.Select
                      value={assignedTo}
